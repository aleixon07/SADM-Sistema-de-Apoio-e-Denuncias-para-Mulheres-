...
<head>
    ...
    <meta name="author" content="Ane - estilização">
...
if (isset($_SESSION['email'])) { //uma validação para verificar se se o usuário está logado
    $sql = "SELECT * FROM administrador"; //uma string com o comando de consulta no banco de dados
    try {
    ...
    if (mysqli_num_rows($result) > 0) {  //se hoveram resultados      
        while($lista = mysqli_fetch_assoc($result)) { //itera sobre todos os resultado, armazenando cada um na variável $lista por iteração                      
            ...
            "<tr> 
                ...
                <td>
                    <a href='./scripts/edit_adm.php?id=$lista[idadministrador]' class='btn btn-primary'>Editar</a>
                                        
                    <!-- Button trigger modal -->
                    <button type='button' class='btn btn-danger' data-bs-toggle='modal' data-bs-target='#excluir_modal_$lista[idadministrador]'>
                    Excluir
                    </button>

                    <!-- Modal Excluir-->
                    <div class='modal fade' id='excluir_modal_$lista[idadministrador]' tabindex='-1' role='dialog' aria-labelledby='excluir_modal_Label' aria-hidden='true'>
                        <div class='modal-dialog' role='document'>
                            <div class='modal-content'>
                                ...
                                <div class='modal-footer'>
                                    ...
                                    <a href='./listar_adm.php?id=$lista[idadministrador]' class='btn btn-danger'>Deletar</a>
                                </div>
                            </div>
                        </div>
                    </div>  
